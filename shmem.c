...
#define ANDROID_SHMEM_GLOBAL_EXECPATH "/data/data/com.termux/files/usr/lib/libandroid-shmem.so"
#define ANDROID_SHMEM_GLOBAL_ENV_EXECPATH getenv("ANDROID_SHMEM_GLOBAL_EXECPATH")
#define ANDROID_SHMEM_GLOBAL_ENABLE getenv("ANDROID_SHMEM_GLOBAL_DISABLE") == NULL
...
#  endif
# endif // LINKER
const char interp[] __attribute__ ((section(".interp"))) = LINKER;
#endif // TEST
...
static int ashv_write_pids(int sendsock, int idx)
{
	if (shmem[idx].countAttach > 0) {
		...
		if ((unsigned int)write(sendsock, &pids, sizeof(pid_t)*shmem[idx].countAttach) != sizeof(pid_t)*shmem[idx].countAttach) {
...
static int ashv_read_pids(int recvsock, int idx)
{
	if (shmem[idx].countAttach > 0) {
		pid_t pids[shmem[idx].countAttach];
		if ((unsigned int)read(recvsock, &pids, sizeof(pid_t)*shmem[idx].countAttach) != sizeof(pid_t)*shmem[idx].countAttach) {
...
static void* ashv_thread_function(void* arg)
{
	...
	while ((sendsock = accept(sock, (struct sockaddr *)&addr, &len)) != -1) {
		...
		if (idx != -1) {
			...
			DBG("%s: ERROR: cannot find shmid %d", __PRETTY_FUNCTION__, shmid);
		}
		if (!global_conf) {
			pthread_mutex_unlock(&mutex);
		}
...
{
	int pid = ashv_socket_id_from_shmid(shmid);
	if (kill(pid, 0) != 0) {
		return -1;
	}
	...
	sprintf(&addr.sun_path[1], ANDROID_SHMEM_SOCKNAME, pid);
...
static int ashv_run_gsocket()
{
	pid_t p = vfork();
	if (p < 0) {
		DBG("%s: ERROR: vfork() failed", __PRETTY_FUNCTION__);
		return -1;
	} else if (p == 0) {
		char* args[] = {(ANDROID_SHMEM_GLOBAL_ENV_EXECPATH) ? ANDROID_SHMEM_GLOBAL_ENV_EXECPATH : ANDROID_SHMEM_GLOBAL_EXECPATH, NULL};
		execvp(args[0], args);
		DBG("%s: ERROR: execvp() failed", __PRETTY_FUNCTION__);
		_exit(1);
	} else {
		int ret = -1;
		...
		if (ret == 0) {
			DBG("%s: global socket successfully started", __PRETTY_FUNCTION__);
		} else {
			DBG("%s: vfork() failed to start global socket", __PRETTY_FUNCTION__);
		}
		return ret;
	}
}
...
int __gsocket()
{
	...
	DBG ("%s: ERROR: listen() failed, gsocket stopped", __PRETTY_FUNCTION__);
	exit(1);
}
...
static int ashv_connect_gsocket(struct sockaddr_un *addr, int action, int shmid)
{
	...
	if (send(gsock, &action, sizeof(action), MSG_NOSIGNAL) != sizeof(action)) {
		DBG("%s: send acction failed on socket %s: %s", __PRETTY_FUNCTION__, addr->sun_path + 1, strerror(errno));
		goto close;
	}
	if (send(gsock, &shmid, sizeof(shmid), MSG_NOSIGNAL) != sizeof(shmid)) {
...
static int ashv_put_shm_gsocket(int shmid)
{
	bool stop = false;
	struct sockaddr_un addr;
	int gsock;
	while((gsock = ashv_connect_gsocket(&addr, ASHV_PUT, shmid)) == -1) {
		if (stop || ashv_run_gsocket() == -1) {
			return -1;
		}
		stop = true;
	}
...
static int ashv_rm_shm_gsocket(int shmid)
{
	struct sockaddr_un addr;
	int gsock = ashv_connect_gsocket(&addr, ASHV_RM, shmid);
	if (gsock == -1) {
		return -1;
	}
...
static int ashv_upd_shm_gsocket(int idx)
{
...
#define FIND_SHMEM \
	int idx = ashv_find_local_index(shmid); \
	if (idx == -1) { \
		if (ANDROID_SHMEM_GLOBAL_ENABLE) { \
			idx = ashv_get_shm_gsocket(shmid); \
		} \
		if (idx == -1 && ashv_socket_id_from_shmid(shmid) != ashv_local_socket_id) { \
...
int shmget(key_t key, size_t size, int flags)
{
	...
	if (key != IPC_PRIVATE) {
		...
		while (true) {
			...
			if (path_length != -1) {
				...
				if (shmid != 0) {
					FIND_SHMEM
					if (idx != -1 && shmem[idx].global && ANDROID_SHMEM_GLOBAL_ENABLE) {
						ashv_upd_shm_gsocket(idx);
	...
	shmem[idx].attachedPids = NULL;
	shmem[idx].global = false;
	if (ANDROID_SHMEM_GLOBAL_ENABLE && ashv_put_shm_gsocket(shmid) == 0) {
		shmem[idx].global = true;
	}
...
#define INIT_SHMEM(ret_err) \
	...
	if (ANDROID_SHMEM_GLOBAL_ENABLE && shmem[idx].global) { \
		ashv_upd_shm_gsocket(idx); \
	} \
	if (shmem[idx].markedForDeletion && shmem[idx].countAttach == 0) { \
...
int shmdt(void const* shmaddr)
{
	...
	for (size_t i = 0; i < shmem_amount; i++)
		if (shmem[i].addr == shmaddr) {
			...
			if (ANDROID_SHMEM_GLOBAL_ENABLE && shmem[i].global) {
				ashv_upd_shm_gsocket(i);
			...
			if (shmem[i].markedForDeletion && shmem[i].countAttach == 0) {
				...
				if (ANDROID_SHMEM_GLOBAL_ENABLE && shmem[i].global) {
					ashv_rm_shm_gsocket(shmem[i].id);
				}
				android_shmem_delete(i);
			}
			pthread_mutex_unlock(&mutex);
